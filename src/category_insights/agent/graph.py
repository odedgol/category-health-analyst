...
from category_insights.agent.intent_extraction import extract_intent
...
from category_insights.domain.models import CategoryMatch, NoteChunk, QueryIntent
...
def build_graph(
    ...
    def extract_intent_node(state: AgentState) -> dict:
        intent = extract_intent(state["raw_question"], adapters.chat_model, state["now"])
    ...
    def fetch_data_node(state: AgentState) -> dict:
        ...
        elif intent.date_range or intent.comparison_range:
        ...
        return {"data_lines": data_lines, "unexpected_delta": unexpected_delta}
...
def answer_question(
    ...
    """Run the full graph for one question and return its answer text.
    ...
    final_state = compiled_graph.invoke(
        {"raw_question": question, "now": now if now is not None else date.today()}
    )
