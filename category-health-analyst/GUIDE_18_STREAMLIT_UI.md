...
is implemented by both the in-memory and DuckDB adapters. `AnalyticsAgent` attaches
that range to `QueryData`, and `expose_requested_metrics()` adds it to the warnings
